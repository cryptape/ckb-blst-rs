                                 ...
                                 size_t n)
{
    ...
    while (((next=u[0]) & 1) == 0) {
        for (next>>=1, count=1; count<LIMB_T_BITS-1; count++, next >>= 1)
            if (next & 1) break;

        for (i=0; i<n-1; i++) {
            limb = next;
            next = u[i+1];
            u[i] = limb | (next << (LIMB_T_BITS-count));
            next >>= count;
        }
        u[i] = next;

        rshift_mod_n(x, x, count, p, n);
    }
}
                                 ...
                                 size_t n)
{
    ...
    if (one != NULL)
        ...
        vec_zero(ux[1], sizeof(ux[1])), ux[1][0]=1;
    vec_copy(vy[0], p, sizeof(vy[0]));
    vec_zero(vy[1], sizeof(vy[1]));

    do {
        remove_powers_of_2_n(ux[0], ux[1], p, n);

        for (borrow=0, i=0; i<n; i++) {
            limbx = ux[0][i] - (vy[0][i] + (llimb_t)borrow);
            borrow = (limb_t)(limbx >> LIMB_T_BITS) & 1;
        }
        if (borrow) {   /* if u<v exchange pointers */
            limb_t (*t)[n] = ux;
            ux = vy;
            vy = t;
        }
        ...
        ret = vec_is_zero(ux[0], sizeof(ux[0]));

        sub_mod_n(ux[1], ux[1], vy[1], p, n);
    } while(ret == 0);

    vec_copy(out, vy[1], sizeof(vy[1]));
    return ret;
}
